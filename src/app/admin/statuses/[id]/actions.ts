...
import { deleteStatus, STATUS_TYPES, type StatusType, updateStatus } from "@/lib/services/statuses";
...
export async function updateStatusAction(formData: FormData): Promise<void> {
  ...
  revalidatePath(`/admin/statuses/${id}`);
...
export async function deleteStatusAction(formData: FormData): Promise<void> {
  ...
  await deleteStatus(id, { db, companyId: adminUser.companyId });
  revalidatePath("/admin/statuses");
