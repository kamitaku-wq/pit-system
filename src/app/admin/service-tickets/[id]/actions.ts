...
import { deleteServiceTicket, updateServiceTicket } from "@/lib/services/service-tickets";
...
export async function updateServiceTicketAction(formData: FormData): Promise<void> {
...
export async function deleteServiceTicketAction(formData: FormData): Promise<void> {
  const adminUser = await getAdminUser();
  if (!adminUser) throw new Error("Unauthorized");
  const id = requiredFormValue(formData, "id");

  await deleteServiceTicket(id, { db, companyId: adminUser.companyId });
  revalidatePath("/admin/service-tickets");
