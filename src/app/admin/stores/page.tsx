...
import { listStores, type StoreListItem } from "@/lib/services/stores";
...
function storeLabel(store: StoreListItem): string {
  return store.name || store.id.slice(0, 8);
}

export default async function AdminStoresPage({ searchParams }: PageProps) {
  const adminUser = await getAdminUser();
  if (!adminUser) redirect("/vendor/login?next=/admin/stores");
  ...
  const { rows, total } = await listStores(
    { page, limit: 20, q: q || undefined, isActive: isActiveFilter },
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-gray-600">店舗情報を管理します。</p>
        </div>
        <Link
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {["店舗名", "コード", "郵便番号", "住所", "電話", "状態", "作成日"].map((heading) => (
                ...
                {rows.map((store) => (
                  <tr key={store.id}>
                    <td className="whitespace-nowrap px-4 py-4 font-medium">
                      <Link className="text-blue-600 hover:text-blue-800 hover:underline" href={`/admin/stores/${store.id}`}>
                        {storeLabel(store)}
                      </Link>
                    </td>
                    <td className="whitespace-nowrap px-4 py-4 text-gray-700">{store.code ?? "-"}</td>
                    <td className="whitespace-nowrap px-4 py-4 text-gray-700">{store.postalCode ?? "-"}</td>
                    <td className="whitespace-nowrap px-4 py-4 text-gray-700">{store.address ?? "-"}</td>
                    <td className="whitespace-nowrap px-4 py-4 text-gray-700">{store.phone ?? "-"}</td>
                    ...
                    <td className="whitespace-nowrap px-4 py-4 text-gray-700">{formatDate(store.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="flex items-center justify-between text-sm text-gray-600">
        ...
        <div className="flex gap-2">
          {page > 1 ? (
            <Link className="rounded-md border border-gray-300 px-3 py-1 hover:bg-gray-50" href={`/admin/stores?${new URLSearchParams({ ...Object.fromEntries(searchBase), page: String(page - 1) })}`}>
              前へ
            </Link>
          ) : null}
          {page < totalPages ? (
