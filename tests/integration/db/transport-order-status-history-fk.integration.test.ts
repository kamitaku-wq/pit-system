...
async function seedFixture(outerTx: Tx, options: { companyLabel?: string } = {}): Promise<Fixture> {
  ...
  const [companyRow] = await outerTx
    .insert(companies)
    .values({ name: `__to_sh_${companyLabel}_${suffix}__`, code: `sh_${suffix}` })
  ...
  const [vehicleRow] = await outerTx
    ...
    .values({
      ...
      vin: `SH${suffix.toUpperCase()}000000`,
    })
...
describeIntegration("transport_order_status_history changed_by_user_id composite FK", () => {
  it("rejects INSERT with cross-company changed_by_user_id via composite FK", async () => {
    await withRollback(async (outerTx) => {
      ...
      await expectPostgresErrorCode(
        () =>
          outerTx.insert(transportOrderStatusHistory).values({
            companyId: fixtureA.companyId,
            transportOrderId,
            toStatusId: fixtureA.statusIds.accepted,
    ...
    await withRollback(async (outerTx) => {
      ...
      const [inserted] = await outerTx
        ...
        .values({
          companyId: fixture.companyId,
          transportOrderId,
          toStatusId: fixture.statusIds.accepted,
      ...
      const rows = await outerTx
        ...
        .where(eq(transportOrderStatusHistory.id, requireRow(inserted, "status history").id));
  ...
  it("accepts INSERT with NULL changed_by_user_id (MATCH SIMPLE)", async () => {
    await withRollback(async (outerTx) => {
      ...
      await outerTx.insert(transportOrderStatusHistory).values({
        companyId: fixture.companyId,
        transportOrderId,
        toStatusId: fixture.statusIds.accepted,
      ...
      const rows = await outerTx
        ...
        .where(
          and(
            eq(transportOrderStatusHistory.transportOrderId, transportOrderId),
            isNull(transportOrderStatusHistory.changedByUserId),
          ),
        );
      expect(rows).toHaveLength(1);
    });
