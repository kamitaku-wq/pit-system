...
- **C.2 sealed + CI green** [2026-05-30]: `confirmTransportOrder` service (service_role 経路で store_confirmed_at 書込, TS-only, no migration) + admin action + UI。Codex review WARN 4 件反映。詳細 `phase-64-c2-manual-confirm-sealed.md`。store admin は service_role `db` 経由ゆえ checkpoint (vendor status_id grant) の影響を受けない。
- **C.3 は実装設計を確定し checkpoint** [2026-05-30]: 調査の結果、(a) vendor session は notification_outbox を直接 INSERT 不可 (`current_user_company_id()` が vendor で NULL → outbox RLS WITH CHECK を満たせない) ゆえ L2-12 完了報告は **SECURITY DEFINER RPC 必須**、(b) checkpoint 解決方針 = 完了 RPC 導入後に `status_id` を vendor GRANT から除去しバイパス封鎖、を確定。security 関連 raw-migration で full gate rigor を要するため、C.0-C.2 完了後の context で急がず **完全な実装設計を `phase-64-c3-vendor-completion-design.md` に保全して checkpoint**。次セッションが fresh context で実装する (着手前検証 → L2-11 → L2-12 RPC + grant 除去 (gate) → UI (Codex 委任) → CI)。
