# Phase B-2: smoke test SQL ドキュメント作成

## ゴール

`phase-handoff/b-2-smoke-test.sql` を新規作成。朝の apply 後に手動実行する smoke test SQL の集約。実行手段は `mcp__supabase__execute_sql` または `psql`。

## ファイル内容 (この全文をそのまま書き出す)
