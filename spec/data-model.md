...
- **availability 検証 (営業時間/定休日/lane 稼働時間/reservation_settings) は本 service に含めない (記録された deferral)**。公開 route で露出する phase (A.30 以降) は、空き枠 picker と共有する availability 検証を本 service 呼び出し前に必ず gate すること (untrusted client 入力対策)。
