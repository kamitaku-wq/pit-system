...
テナント境界: 公開 URL の `companyId` (UUID) が唯一の company scope。全 read で `companyId` と store/menu/lane の `company_id` 一致を検証 (URL 改竄防御)。menu 可視性は slots エンドポイントでも独立検証 (非公開メニューの枠直取り防止、defense-in-depth)。GET route = `/r/reserve/[companyId]/slots` (純 read、GET-safe)。**POST gate→create 配線 + multi-step UI は A.31b。**
