...
| Main | 顧客 facing セキュリティレビュー (#1): `/r/[token]/*`・予約フロー・token 発行を security-reviewer agent でレビュー | HIGH 以上の指摘ゼロ (修正済) |

### 2.4 Sprint β-4 (6/23-29, 1 週): Phase 4 通知拡張 + リリース準備

> **Lane 依存順序 (Phase 64-A.26 workflow 最適化 #6)**: channel 拡張 migration は Lane Main 専管で Codex 委任不可。Day1 に Lane Main で先行し、Lane A の LINE/SMS 実装はこれを前提に並列着手する。

| Lane | タスク | 完了基準 |
|---|---|---|
| Main (Day1) | notification_outbox.channel の CHECK 制約を ALTER TABLE で email → email/line/sms に拡張 + dispatcher の channel routing インターフェース (email 以外の handler を受ける構造) 設計確定。※具体 enum 値は β-4 着手時に確定 | migration 適用 + routing IF 確定 |
| Main | PII 匿名化 cron (pii_anonymization_jobs): 期限切れデータの匿名化 | 匿名化実行確認 |
| Main | 経理証跡 view: billing_records + 月次集計 | SELECT 動作確認 |
| A (Day2-) | LINE/SMS 通知拡張: Twilio Webhook 統合 (Day1 channel migration を前提) | テスト送信確認 |
| A (Day2-) | notification_outbox チャネル routing 実装: email / line / sms | 各チャネル送信確認 |
